...
MsgBox.prototype.create = function(){
  ...
  var domElement = $('<div></div>')
    ...
    [ this.options.text ? 'text' : 'html' ](  this.options.massage  )
      ...
      //  custom handlers
   .bind({
      'closed.msgBox.Event'  : $.proxy(this.options.callAfterAlert, this.options , 
         this.count, this.options.type) , 
      'before.msgBox.Event'  : $.proxy(this.options.callBeforeAlert, this.options ,
         this.count, this.options.type ) 
    })
  ...
  $( this.root ).append( domElement );
    $.support.transition && this.options.animation ? 
     domElement
      .addClass('animated ' + this.cssFloats[default_option.global.float])
...
MsgBox.prototype.container = function() { 
 !defined && 
   ...
   $('.msgbox_container').css({'bottom': default_option.global.bottom, 'left': '', 'right': ''}
      ).css( default_option.global.float , default_option.global.offsets 
    )
  defined = true;
}
...
// global export 
  ...
  $.msgBox = function ( option ) {
      return  !default_option.massage ?  Throw(errors['msg']) : 
